...
class Keygen:
    """
    Anahtar üretmek için bir sınıf kullanıcı anahtar seçimi yapamaz rastgele üretilir ve bu PBDKF2 ile türetilir ve iv'de seçemez bu da rastgele üretilir 
    anahtarı açmak için tuz dosyasına ve şifreye ihtiyacı vardır bunları bu sınıf üretir Salt dosyası .Qslt ve .Qkey dosyası
    """
    def __init__(self):
        self.iv = secrets.token_bytes(16)
        self.key = PBDKF2(secrets.token_bytes(256)).key
        self.hmac_key = os.urandom(128)
    def __repr__(self) -> str:
        return "<%s key = %s iv = %s hmac key = %s>" % (self.__class__.__name__,self.key,self.iv,self.hmac_key)
